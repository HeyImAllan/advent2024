...
public class day20 {
    ...
    public static void part1() {
        ...
        int INF = 999999999;
        int answerp1 = 0;
        int k = 2; // Batch size for removing walls
        Combinations combinations = new Combinations(walls.size(), k);
        for (int[] combination : combinations) {
                List<Point> testNodes = new ArrayList<>(nodes);
            Map<Point, String> newmap = new HashMap<>(map);
            for (int index : combination) {
                Point p = walls.get(index);
                if (p.x != 0 && p.y != 0 && p.x != mapHeight - 1 && p.y != mapWidth - 1) {
                testNodes.add(nodes.size() - 2, p);
                    newmap = updateMap(p, ".", newmap);
                }
            }
